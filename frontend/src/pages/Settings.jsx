...
import { clearPageTitleCache, usePageTitle } from '../hooks/usePageTitle.js';
...
function IpWhitelistSection() {
  ...
  const handleAdd = async () => {
    ...
    if (!trimmed) return;
    if (!validateIp(trimmed)) { alert(t('settings.ipWhitelistInvalid')); return; }
    if (ips.includes(trimmed)) { alert(t('settings.ipWhitelistExists')); return; }
  ...
  return (
    <div style={{ background:'#fff', border:'1px solid #e2e6ed', borderRadius:10, overflow:'hidden', marginTop:24 }}>
      <div style={{ padding:'16px 24px', borderBottom:'1px solid #e2e6ed', background:'#f8f9fb' }}>
        <h2 style={{ fontSize:16, fontWeight:600, color:'#1a1d23', marginBottom:2 }}>🔒 {t('settings.ipWhitelist')}</h2>
        <p style={{ fontSize:13, color:'#6b7280' }}>{t('settings.ipWhitelistDesc')}</p>
      </div>
      <div style={{ padding:'20px 24px' }}>
          ...
          <>
            <div style={{ display:'flex', gap:8, marginBottom:16 }}>
              <input
                ...
                style={{ flex:1, padding:'8px 12px', borderRadius:7, border:'1px solid #e2e6ed', fontSize:13, outline:'none', textAlign: isRTL ? 'right' : 'left', direction:'ltr' }}
              />
              <button onClick={handleAdd} disabled={saving || !newIp.trim()}
                style={{ padding:'8px 16px', borderRadius:7, border:'none', background:'#1a1d23', color:'#fff', fontSize:13, cursor:'pointer', fontWeight:500 }}>
                {t('settings.ipWhitelistAdd')}
              </button>
            </div>
            {ips.length === 0 ? (
              <div style={{ color:'#9ca3af', fontSize:13, fontStyle:'italic', padding:'8px 0' }}>
                {t('settings.ipWhitelistEmpty')}
              </div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {ips.map(ip => (
                  <div key={ip} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', background:'#f8f9fb', borderRadius:8, border:'1px solid #e2e6ed' }}>
                    <span style={{ fontFamily:'monospace', fontSize:14, color:'#1a1d23', direction:'ltr' }}>🌐 {ip}</span>
            ...
            {saved && (
              <div style={{ marginTop:12, color:'#16a34a', fontSize:13, fontWeight:500 }}>✓ {t('settings.ipWhitelistSaved')}</div>
            )}
          </>
        )}
      </div>
        ...
        <div style={{ padding:'16px 24px', borderBottom:'1px solid #e2e6ed', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#f8f9fb' }}>
          ...
          {isAdmin && (
            <button onClick={()=>setModal({type:'add'})} style={{...btnStyle, background:'#1a1d23', color:'#fff'}}>
              <Plus size={15}/> {t('settings.addUser')}
            </button>
          )}
        </div>
